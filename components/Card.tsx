...
function Card({ images, title, description, level, destination, snap }: Cardprops) {
  ...
  return (
    <div onMouseOver={() => setMouseOver(true)} onMouseOut={() => setMouseOver(false)} id='card' ref={ref} onClick={navigate} className={`${styles.Card} group w-[87%] max-w-[300px] z-[1] sm:z-[1] h-[95%] max-h-[500px] flex-none cursor-pointer ${snap} rounded-[20px] mx-[15px] my-[10px] shadow-task last:mr-[30px] p-[5%] flex flex-col sm:h-[70vh] sm:max-h-[600px] sm:rounded-[25px] sm:w-[88%] sm:hover:z-1  sm:hover:scale-110 duration-700`}>
      {/*------- challenge image ----- */}
      <Carousel showThumbs={false} autoPlay={autoPlaySetings()} infiniteLoop={((intersecting && width < 600) || (mouseOver && width > 900)) ? true : false} showArrows={false}>
        {images.map((item, index) => (
          <img key={index} src={item} alt="" className={`${styles.CardImg} rounded-[15px] max-h-[60%] w-[100%] min-h-[40vh] sm:max-h-[65%]`} style={{ boxShadow: "rgba(0, 0, 0, 0.12) 0px 1px 3px, rgba(0, 0, 0, 0.24) 0px 1px 2px" }} />
